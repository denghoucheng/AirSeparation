...
<head>
	...
	<style type="text/css">
		*{margin:0; padding: 0;}
		body{font-family: "微软雅黑";font-size: 12px;color:#666;background: #E6E6E6; border:2px solid red;}
	...
	<script type="text/javascript">
		function openTab(systemName,url,iconCls){
			...
			if($("#tabs").tabs("exists",systemName)){
		        ...
		        var content="<iframe frameborder=0 scrolling='auto' style='width:100%;height:90%' src='${pageContext.request.contextPath}/"+url+"'></iframe>";
		        console.log(content);
		        $("#tabs").tabs("add",{
		            title:systemName,
		            iconCls:iconCls,
		...
		<c:forEach items="${factoryInfoList}" var="facCurInfo">
			<c:choose>
				<c:when test="${facCurInfo.systemName == '报警系统'}">
					<a href="javascript:openTab('${facCurInfo.systemName}','alarm.html?factoryId=${facCurInfo.factoryId}')" class="easyui-linkbutton" data-options="plain:true" id="btn">${facCurInfo.systemName}</a>
				</c:when>
				<c:otherwise>
			 		<a href="javascript:openTab('${facCurInfo.systemName}','systemCurrent.html?modelId=${facCurInfo.modelId}&modelName=tb2_model${facCurInfo.modelNum}&systemName=${facCurInfo.systemName}')" class="easyui-linkbutton" data-options="plain:true" id="btn">${facCurInfo.systemName}</a>
				</c:otherwise>
			</c:choose>
		</c:forEach>
	</div>
